...
module ActionView::Helpers::CalendarHelper
  ...
  def calendar_field_tag(name, date, show_field = true, popup = true, html_options = {}, calendar_options = {})
    ...
    date_converted = date.to_time.strftime(calendar_options[:ifFormat]) rescue ""
    add_defaults(field_options,
      :value => (date && date != "" && date_converted) || "",
      :size => 12
    )
    rename_option(field_options, :field_title, :title)
    remove_option(field_options, :button_title)
    if show_field
      field = text_field_tag(name, date.to_s, field_options)
    else
      field = hidden_field_tag(name, date.to_s, field_options)
    end

    input_field_id = name
    calendar_id = "#{name}_calendar"

    if popup
      button_options = html_options.dup
      button_options.store(:plugin,"bagel")
...
END
  ...
  def _calendar(object, method, show_field = true, popup = true, html_options = {}, calendar_options = {})
	...
	#FIXME:
	if object.match(/\[/)
	  input_field_id = "#{object_name}_#{value(object_name,"id")}_#{method}" 
	  calendar_id = "#{object_name}_#{value(object_name,"id")}_#{method}_calendar" 
	else
	  input_field_id = "#{object_name}_#{method}" 
	  calendar_id = "#{object_name}_#{method}_calendar" 
	end
