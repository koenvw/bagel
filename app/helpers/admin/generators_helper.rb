module Admin::GeneratorsHelper
  def render_subtree(categories, root_id, output='')
    ...
    children.each do |child|
      # Open list item tag
      output << tag('li', { :id => 'node' + child.id.to_s, :noDrag =>"true" }, true)

      # Render subtree
      output << link_to(h(child.name), { :action => 'list', :id => child.id, :website_id => params[:website_id] }, :id => 'cat_' + child.id.to_s)
